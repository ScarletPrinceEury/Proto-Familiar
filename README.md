...
- **End a topic** — click the **⬛** end button that appears on any message while hovering. If multiple open topics include that message, a picker appears.
- **Retroactive start** — click the **▷** start button on any past message to begin a topic from that point instead of the present.
...
- **Auto-summary** — when a topic ends, a summary is generated by the LLM. You can edit the title, body, and keywords, then save it to the Lorebook as a new entry with those keywords as triggers.
