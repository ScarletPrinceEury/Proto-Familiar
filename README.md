...
| **Silence triage & trusted-contact outreach** | When threat is elevated AND you've been silent past a tier threshold (severe=15min, high=1h, moderate=4h), a 5-min triage loop asks the LLM "should I reach out, gently?" The LLM decides — `wait` is honored. On reach-out: warm 1–2 sentence note in the outbox. Optionally, when you've configured **trusted contacts** (sidebar section, Discord-webhook based), the LLM may *also* suggest contacting one by name; the system delivers via webhook AND duplicates the exact message into your outbox so there is no covert contact. Hard-disable with `PROTO_FAMILIAR_TRIAGE_DISABLED=1` |
