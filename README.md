...
**Thalamus** is the bridge between you and everything above. Every message you send passes through it — it gathers the relevant memories, identity context, and time awareness that make each response grounded rather than generic. It also checks who the Familiar is talking to, so it can make sure their memory blanks on information they aren't allowed to pass on to specific people.too
