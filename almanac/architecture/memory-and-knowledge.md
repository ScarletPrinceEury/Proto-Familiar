...
2. **Session end**: [When a session ends](session-lifecycle) — normally, or via the manual close-out for a session that never received an `endedAt` — the browser or Discord fires an [autonomous memorization job](session-memorization) with `POST /api/memorize`, enqueuing one or more summary tasks.
3. **Job processes**: The memorization worker (a 5-second-tick background loop) picks up jobs and calls the LLM to summarize a conversation slice into lorebook entries. Each candidate fact passes the [memory-integrity gate](memory-integrity) — a scan for corrupted or adversarial phrasing, separate from ordinary consent — before it is written; a suspect fact from an untrusted source is held in a reversible quarantine instead. Entries that pass are written to the [Session Memories](tomes-and-lore) Tome.
4. **Consolidation**: [Phylactery's consolidation pipeline](phylactery) sweeps the daily-granularity memories and rolls them up weekly, monthly, and yearly as they age. The ward-configured "load-bearing versus decorative" criterion decides what survives. The ward can also force this pass immediately — see [Pondering](pondering)'s on-demand consolidation section for the Discord command and UI button that trigger it (and the matching one for the ponderings tome) on demand.
