...
| **Tool calling** | LLM can invoke 25 built-in tools; the registry, executors, and multi-round loop (up to 5 rounds) run server-side in `cerebellum.js`. Tools span knowledge management (`save_to_tome`, `save_memory`, `update_identity`, graph CRUD), temporal context (schedule, reminders, interests), and crisis outreach (`get_trusted_contacts`, `contact_trusted_person`, `show_crisis_resources`). See [docs/tool-calling.md](docs/tool-calling.md) |
