...
| **Autonomous pondering** | The Familiar wakes on its own cadence during idle periods, picks an interest with weight-proportional sampling, ponders it via the LLM, and writes a real timestamped entry to the **Familiar's Ponderings** tome. Cadence (30 min – 6 hr) shortens when interest weight is high; ponderings get injected into chat context the next time you talk, so the Familiar can reference its own real thoughts honestly. Toggle in Settings → "Autonomous pondering"; stretch intervals via "Pondering interval scale"; hard-disable with `PROTO_FAMILIAR_PONDERING_DISABLED=1`. **Unattended web research (§8.5):** when web search is on, a ponder tick can look a few things up first — the model NAMES what to search/read, code does the bounded reads (never a tool-calling loop: less agency when unattended). A shared per-day read budget (`ponderWebReadsPerDay`, default 12) caps spend; an exhausted budget just means no lookups that tick (the prompt says so — never a failed call). Reads are audited under surface `pondering`. Toggle "Look things up while pondering"; hard-disable with `PROTO_FAMILIAR_PONDER_WEB_DISABLED=1`. See [`docs/caring-spine-build-plan.md`](docs/caring-spine-build-plan.md) |
...
| **Reminders & outbox** | Schedule-layer nodes of type `reminder` fire on their `when_ts` via a 30s background scheduler. Fired reminders enqueue into an outbox; the chat UI injects them as ordinary assistant chat messages in the active session. Same outbox is used for autonomous silence-triage check-ins and trusted-contact outreach. Hard-disable with `PROTO_FAMILIAR_REMINDERS_DISABLED=1` |
| **Discord push notifications** | Set **My Discord webhook** (sidebar → Trusted contacts) and every outbox delivery — reminders, check-ins, outreach notices, crisis resources — is *also* pushed to your own Discord, so your Familiar can still reach you when no tab is open. Per-channel delivery state is recorded on each item, and the trusted-contact escalation countdown only starts once a check-in is confirmed delivered |
...
| **Tool calling** | LLM can invoke a large and growing set of built-in tools (well past the original 25 as vision, temporal, village, and content-gating tools landed); the registry, executors, and multi-round loop run server-side in `cerebellum.js`. Round budget is per-surface: background deliberations and non-ward Discord turns cap at `MAX_TOOL_ROUNDS` (5), while a live ward turn (web or ward Discord) uses the ward-configurable `toolRoundsPerTurn` (default 12, clamp [3,30]) — see [docs/tool-calling.md](docs/tool-calling.md). Tools span knowledge management (`save_to_tome`, `save_memory`, `update_identity`, graph CRUD), temporal context (schedule, reminders, interests), and crisis outreach (`get_trusted_contacts`, `contact_trusted_person`, `show_crisis_resources`) |
| **Custom tools** | Paste a JSON array of OpenAI-compatible function definitions; advertise-only — the model sees and may call them, but calls return a "not implemented" notice (real executors are a planned extension point) |
...
| **Auto-sync across devices** | On startup the client asks `GET /api/active-session` for the most-recently-updated session on the server. An empty local chat resumes a recent (<48 h) server session silently — switch devices and the conversation is just *there*. When the local chat has messages, or the server session is older, a dismissible **Resume** banner offers the choice instead of overwriting |
...
The **Tools** section in the sidebar controls the Familiar's tool use. Since 0.4.0-alpha the tool registry, the executors, and the multi-round loop all run **server-side** (in `cerebellum.js`); the browser only renders the collapsible call/result blocks.

#### Enabling / disabling

The **Enable tool use** checkbox controls whether each request opts into the server-side tool loop. When unchecked, no tools are advertised to the model and it behaves as a plain chat completion.
...
Custom tools are advertised to the LLM like built-in tools, but they are **advertise-only**: a call returns a notice that the tool has no implementation yet. Use custom definitions to let the model *describe* what it would do, or extend `TOOL_EXECUTORS` in `cerebellum.js` to wire real logic.

#### How the loop works

1. The browser sends one `/api/chat` request with `runToolLoop: true`; the server composes the tools array (built-ins + custom) with `tool_choice: 'auto'`.
2. If the provider's response has `finish_reason: 'tool_calls'`, the server executes each call via `cerebellum.executeToolCall`.
3. The round is streamed to the browser as a `_toolRound` event, rendered as a compact collapsible block showing the call name, arguments, and result.
4. The assistant message + tool results are appended and the provider is re-called — all inside the same request.
...
```
/
├── server.js                    Express server — chat proxy + log/tome/memorize/entity API (Node.js 22+, ESM)
