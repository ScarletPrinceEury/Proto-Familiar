...
├── websearch.js             Web access (opt-in, 0.7.0) — backs the look_up / web_search / read_webpage tools. look_up (0.7.19) answers definitions/facts/overviews from keyless official reference APIs (Wikipedia action API + DuckDuckGo Instant Answer API), no scraping/no setup. web_search finds pages; searchWeb resolves the human-chosen backend (0.7.20): custom webSearchBaseUrl wins; else webSearchBackend ∈ {basic → in-box keyless DuckDuckGo HTML scrape; api → a provider adapter (Brave/Tavily/Google) via webSearchApiProvider+key; local → the active managed engine via deps.managedSearch, which local-engine-service injects (0.7.24) so web search uses whichever engine is running without knowing its JSON dialect}. ANY backend failing falls through to the keyless floor — a wrong key / down instance never leaves the human without search. Each search logs which backend actually served (`[websearch] "q" — served via …` / `… unavailable; fell back to built-in keyless search`) so a silent fallback is visible (0.7.28). Owns the SSRF guard (scheme allow-list + resolved-IP block of loopback/private/link-local/metadata + redirect re-validation), the fetch timeout, and the linkedom→@mozilla/readability→turndown extraction with provenance stamping. cerebellum registers the defs + delegates; gated by webSearchEnabled / PROTO_FAMILIAR_WEBSEARCH_DISABLED=1 (see docs/websearch-setup.md, docs/websearch-modular-build-spec.md)
├── websearch-providers.js   Proper search-API adapters (0.7.20) — braveSearch / tavilySearch / googleSearch, one small JSON client each (no scraping), all returning the same {rows}|{error} shape searchWeb dispatches through (API_PROVIDERS registry). Provider hosts are sanctioned public endpoints (not the SSRF-guarded read path); missing/bad key → {error} → searchWeb degrades to the floor
├── php-runtime.js           Fetch a PHP CLI runtime on demand for the PHP-based engines (4get/LibreY) (0.7.25; Windows 0.7.27) — fetched on first install (gitignored vendor/php-runtime/), two sources by host: Linux/macOS (x86_64/aarch64) get a self-contained static-php-cli "common" binary; Windows x64 gets the official windows.php.net NTS zip + a generated php.ini enabling the DLL extensions (curl/openssl/mbstring/gd/…) + a fetched cacert.pem wired via curl.cainfo/openssl.cafile (Windows PHP ships no CA bundle, so curl HTTPS — the engines' scraping — would otherwise fail and 500). ensureWindowsConfig backfills the bundle+ini on existing installs too (0.7.33). phpSpec() is the per-host source-of-truth; phpSupported() is false only on 32-bit/exotic hosts. Integrity: HTTPS + pinned-version URL (+ archives/ fallback on Windows) + functional `php --version` check, optional hard PHP_SHA256 pin. ensurePhp() throws → engine degrades to keyless, never breaks search
...
├── local-engine-adapters.js Query a running LOCAL engine's JSON API (0.7.24; 4get/LibreY 0.7.26) — searxngSearch (/search?format=json), libreySearch (/api.php?q=&type=text → JSON array), fourgetSearch (/api/v1/web?s= → web[]), each turning "engine base URL + query" into the uniform {rows}|{error} shape via the shared timedFetch. Used by local-engine-service (the active managed engine) and by websearch.js (the power-user bring-your-own-SearXNG URL). One adapter per dialect, no copy-paste
├── web-fetch-util.js        timedFetch — a shared fetch-with-AbortController-timeout used by the SearXNG JSON path and the provider adapters (one place, not copy-pasted). Not the SSRF guard; just the timeout + JSON defaults for sanctioned backends
├── local-engine-service.js  Familiar-managed LOCAL search engines (0.7.x; engine-agnostic 0.7.21). One 30s supervisor (like the Discord gateway), MANY possible engines but only ONE running — the engine the ward selected (webSearchLocalEngine) while the 'local' backend is active. Each engine is a DESCRIPTOR (installed/ensureInstalled/spawn/uninstall); the supervisor is generic. SearXNG is the first descriptor (Python/uv; third-party + NOT committed — FETCHED on first install, pinned to SEARXNG_PIN, tracked patches under vendor/searxng-patches/ re-applied, then uv-spawned `python -m searx.webapp` with generated loopback+JSON settings.yml); 4get/LibreY (PHP) join in Part 3. desiredEngine() picks from settings; reconcile() converges (switching tears the old down first); managedEngineSearch() queries the active engine in its own JSON dialect (cerebellum injects it as searchWeb's managedSearch) — a cold/failed/absent/switching engine returns {error} and ALWAYS degrades to keyless, never breaking search. installEngine/uninstallEngine/localEngineStatus back the Settings modal (Part 2c). Hard off-switch PROTO_FAMILIAR_LOCAL_ENGINE_DISABLED=1 (alias …_SEARXNG_DISABLED=1). See docs/websearch-modular-build-spec.md
