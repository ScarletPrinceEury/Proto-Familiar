...
├── php-runtime.js           Fetch a self-contained static PHP CLI binary on demand (0.7.25) — the PHP-based engines (4get/LibreY) need a PHP runtime we don't ship, so it's FETCHED on first install (static-php-cli "common" build, pinned PHP_VERSION, from dl.static-php.dev) into vendor/php-runtime/ (gitignored), same fetch-on-enable pattern as SearXNG source. Linux/macOS (x86_64/aarch64) only — phpSupported() is false on Windows, where the PHP engines degrade to unavailable (SearXNG via uv still works). Integrity: HTTPS + pinned-version URL + functional `php --version` check, with an optional hard PHP_SHA256 pin. ensurePhp() throws → engine degrades to keyless, never breaks search
├── local-engine-adapters.js Query a running LOCAL engine's JSON API (0.7.24) — searxngSearch (and 4get/LibreY in Part 3), each turning "engine base URL + query" into the uniform {rows}|{error} shape via the shared timedFetch. Used by local-engine-service (the active managed engine) and by websearch.js (the power-user bring-your-own-SearXNG URL). One adapter per dialect, no copy-paste
