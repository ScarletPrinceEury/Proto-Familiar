...
When enabled (Settings → Tools → **Web search & read**), the Familiar gains three tools: `look_up` answers a definition/fact/overview question from keyless official reference APIs (Wikipedia + the DuckDuckGo Instant Answer API — no scraping, no setup); `web_search` finds pages out on the web; and `read_webpage` opens a result and reads it as clean markdown. It works out of the box with a built-in keyless backend — nothing to install. `read_webpage` is SSRF-guarded (public http/https only — loopback, private, link-local, and cloud-metadata targets are refused), times out on slow hosts, and stamps each read with its source URL and date. A page stays in the conversation for the session; the Familiar keeps anything worth remembering past it via `save_to_tome`. **Finding pages** (`web_search`) works out of the box (a built-in keyless DuckDuckGo backend) and can be upgraded via **Settings → Web search → Configure search backend** to a proper search **API**: **Marginalia** (an independent small-web index — no account, no card, no key at all), **Tavily** (no card), **Brave** (independent index, needs a card), or **Google**. Whatever you pick, a wrong key or a down provider always falls back to the built-in search — you're never left without it. Not sure which to pick? The modal has a **"Ask me about these options"** mini-chat — the same Familiar, in its own voice, explaining the trade-offs and walking you through signup in plain language. Hard-disable web access with `PROTO_FAMILIAR_WEBSEARCH_DISABLED=1`; the in-modal guide with `PROTO_FAMILIAR_GUIDE_CHAT_DISABLED=1`.
...
- **Graph** — two view modes via the toolbar's List / Map toggle.
  ...
  - **Map view** renders the entire graph as dots and quadratic curves on a canvas (via the shared `graph-map.js` engine, also used by the schedule map). Node hue encodes type (deterministic per-graph palette with a stride-of-7 across 24 hues so adjacent type names land 105° apart, not in adjacent buckets); edge hue encodes relationship type, and edge saturation / lightness / alpha scale with the edge's weight (`0.0 → 1.0`) so strong relationships read as vivid and weak ones fade. **Pan** by dragging or one-finger; **zoom** by wheel, two-finger pinch, or the **＋ / − / Fit** buttons (the touchpad-friendly path — no scroll wheel required). Hover surfaces a tooltip (label + type + description for nodes, endpoints + weight for edges, hit-tested against the actual Bézier curve), and labels for every dot appear past ~1.4× zoom. A legend lists every node type and edge type currently on screen.
...
**The consequence graph (Schedule → Map).** Unruh is a graph, not a flat list — the reason a graph was chosen is so events can *relate to each other* and the Familiar can reason about consequence: the interview **causes** the anxiety, the prep **is required** before it, the late plans **block** an early start. The Temporal editor's Schedule tab has a **Map** view (alongside List and Calendar) that draws this graph on the same canvas as the knowledge map: nodes are events/tasks/phases/states (resolved ones faded), edges are the `causes`/`requires`/`depends_on`/`blocks`/`during`/`carries_forward` links, hued by kind. Click a node to see its links and **+ connect** it to another item (target + kind). The Familiar authors the same links from its side with the `schedule_link` tool — so the graph fills in from both directions.

**Consequences over time.** An edge can carry a *consequence*: whether it **helps or harms**, which future it belongs to (`on_resolve` = what finishing buys you / `on_lapse` = what skipping costs), roughly when it lands, how severe, and how sure the Familiar is (a projection wears its **certainty**; an `observed` one is a fact). On the Map, harm reads red, help green, and a **projected** consequence is dashed where an **observed** one is solid. The Familiar uses this to plan: a task an imminent thing depends on, or that carries a soon high-stakes cost, surfaces harder and with the *reason* named. And it keeps honest score — when a task resolves it records *where in its time window* you acted, and the reflection loop both learns your timing patterns and grades its own past forecasts (raising or lowering their certainty). See [`consequence-graph-build-spec.md`](consequence-graph-build-spec.md).
