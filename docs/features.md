...
| Web search & read | Opt-in web access (`look_up` + `web_search` + `read_webpage`). `look_up` answers definitions/facts from keyless reference APIs (no scraping); `web_search` finds pages. Works out of the box (built-in keyless search) — off by default, just turn it on. Optionally point it at your own SearXNG. See [Web search setup](websearch-setup.md) |
...
When enabled (Settings → Tools → **Web search & read**), the Familiar gains three tools: `look_up` answers a definition/fact/overview question from keyless official reference APIs (Wikipedia + the DuckDuckGo Instant Answer API — no scraping, no setup); `web_search` finds pages out on the web; and `read_webpage` opens a result and reads it as clean markdown. It works out of the box with a built-in keyless backend — nothing to install. `read_webpage` is SSRF-guarded (public http/https only — loopback, private, link-local, and cloud-metadata targets are refused), times out on slow hosts, and stamps each read with its source URL and date. A page stays in the conversation for the session; the Familiar keeps anything worth remembering past it via `save_to_tome`. **Finding pages** (`web_search`) works out of the box (a built-in keyless backend) and can be upgraded via **Settings → Web search → Configure search backend**: a proper search **API** (Tavily — no card; Brave; or Google), or a **local engine** the Familiar installs and runs on your machine (SearXNG now; lighter 4get/LibreY coming). Whatever you pick, a wrong key or a down engine always falls back to the built-in search — you're never left without it. Power users can also point search at their own [SearXNG](https://docs.searxng.org/) — see [Web search setup](websearch-setup.md). Hard-disable web access with `PROTO_FAMILIAR_WEBSEARCH_DISABLED=1`; the managed local engines with `PROTO_FAMILIAR_LOCAL_ENGINE_DISABLED=1`.
