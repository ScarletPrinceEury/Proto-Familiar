...
> libraries `linkedom`, `@mozilla/readability`, `turndown`), installed by the `npm install`
...
> its libraries aren't installed until you run the installer. Web search works keyless out of the
> box (DuckDuckGo) and can optionally use a search API (Marginalia needs no signup) — nothing is
> installed or spawned locally.
