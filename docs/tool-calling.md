...
Two further tools — `web_search` and `read_webpage` — are **opt-in** (since **0.7.0-alpha**): they appear in the tool list only when the human enables web access in Settings, and the `PROTO_FAMILIAR_WEBSEARCH_DISABLED=1` env var forces them off regardless. They are the only built-ins gated this way; everything else above is always present. Search works **out of the box** — the default backend is a built-in keyless DuckDuckGo scrape (no install); a power user can optionally point `webSearchBaseUrl` at their own SearXNG (see [`websearch-setup.md`](websearch-setup.md)). All web logic lives in `websearch.js` (the SSRF guard, the timeout, the search backends, the `linkedom`+`@mozilla/readability`+`turndown` extraction); cerebellum only registers the defs and delegates.
...
| `save_memory` | Write a time-stamped memory (a moment/event/pattern with a *when*) at a chosen granularity (`daily` \| `weekly` \| `monthly` \| `yearly` \| `significant`). Recall-first: correct in place if mis-recorded, or supersede with a fresh dated entry if it evolved | `"Memory saved."` or an error string |
...
| `web_search` *(opt-in)* | Search the web (`query`). Returns the top N results (default 5) as titles + links + snippets. Built-in keyless backend by default (no setup); optional SearXNG via `webSearchBaseUrl`. Only advertised when web access is enabled in Settings | Compact result list, or a calm failure string if the web is unreachable |
