...
| `schedule_link` | Connect two scheduled items into Unruh's **consequence graph**: `src` {`causes`\|`requires`\|`depends_on`\|`blocks`\|`during`\|`carries_forward`\|`co_occurs_with`} `dst`. Optionally records a **consequence**: `valence`, `condition` (`on_resolve`/`on_lapse` — the two futures), `horizon_hours`, `severity`, `certainty`, `observed`, `note`; `dst_state` resolve-or-creates a consequence-state node by label. The src id comes from the `[schedule ids]` legend | Confirmation string |
