...
| `schedule_link` | Connect two scheduled items into Unruh's **consequence graph** so the Familiar reasons about how they bear on each other: `src` {`causes`\|`requires`\|`depends_on`\|`blocks`\|`during`\|`carries_forward`} `dst`. Both ids come from the `[schedule ids]` legend. This is the authoring half of the graph the schedule was always shaped to hold | Confirmation string |
