...
- **Web search backend (`searxng-service.js`, 0.7.x)**: web search works out of the box via a keyless in-process backend (`websearch.js` → DuckDuckGo HTML scrape) — the always-available floor. **Optionally**, a heavier-duty self-hosted [SearXNG](https://github.com/searxng/searxng) is run by the Familiar itself: spun up when my human turns on "Web search & read" and torn down when they turn it off (same toggle-followed lifecycle as the Discord gateway). It is a **vendored third-party app** (lives under `./vendor/searxng/`, uv-managed) — NOT our code like Phylactery/Unruh. The managed instance always degrades to keyless: a cold/failed spawn never breaks search. Hard off-switch `PROTO_FAMILIAR_SEARXNG_DISABLED=1` (keyless still works).
    - ⚠️ **Vendored-SearXNG update cadence:** because `./vendor/searxng/` is someone else's actively-developed, web-facing app, **check for upstream SearXNG updates to pull on every MINOR (`0.X.0`) or MAJOR (`X.0.0`) version bump** of Proto-Familiar — and pull security-relevant changes promptly. Leave it untouched on PATCH bumps (`0.0.X`). SearXNG is a **rolling release** (no version tags — every commit to master is a "release"), so we pin to a specific **commit SHA**, recorded in `./vendor/searxng/VERSION`. Moving the pin is deliberate, never live-tracking master; and because behaviour can drift between commits, **re-run the spawn smoke-test whenever the pin moves** (the `searxng-service.js` entrypoint/env/health assumptions can change).
- **Autonomous loops**: seven background workers run alongside the HTTP server. Each has a settings/env hard off-switch:
    ...
    - **Tome → Phylactery graduation** (`tome-graduation-loop.js` + `tome-graduation.js`, Phase 4 of Phylactery-ingestion) — the autonomous pass that drains durable facts *stranded* in tomes into their right canonical home. **Opt-in (default OFF)** — it writes to the canonical self, so it idles until the ward enables "Graduate tome knowledge" in Settings. Slow 30-min tick: code-gated candidate selection (skips Ponderings / Session Memories / runtime tomes + already-reviewed entries) → ONE batched LLM judgment reusing the Phase-3 routing rubric (unsure → *stays a tome*) → routes via thalamus wrappers (ward long-term memory through the consent greenlight) → tidies the entry **only after a confirmed route** (`tomeGraduationTidy`: `delete` declutter / `pointer` breadcrumb). **v1 routes to identity + memory only**; autonomous graph construction is deferred (the one risky route). Distinct from Pillar H graduation (identity→RAG). Hard-disable with `PROTO_FAMILIAR_TOME_GRADUATION_DISABLED=1`. See `docs/tome-graduation-build-spec.md`.
