...
- **Web search backend (`searxng-service.js`, 0.7.x)**: web search works out of the box via a keyless in-process backend (`websearch.js` → DuckDuckGo HTML scrape) — the always-available floor. **Optionally**, a heavier-duty self-hosted [SearXNG](https://github.com/searxng/searxng) is run by the Familiar itself: spun up when my human turns on "Web search & read" and torn down when they turn it off (same toggle-followed lifecycle as the Discord gateway). It is third-party code — NOT our code like Phylactery/Unruh — and is **NOT committed** (its ~970-file source bloated the repo). It is **fetched on first enable**, pinned to a commit SHA (`SEARXNG_PIN` in `searxng-service.js`), with the tracked patches under `./vendor/searxng-patches/` re-applied; the fetched tree is gitignored. A missing `git`/network, cold, or failed spawn always degrades to keyless — never breaks search. Hard off-switch `PROTO_FAMILIAR_SEARXNG_DISABLED=1` (keyless still works).
    - ⚠️ **SearXNG pin update cadence:** SearXNG is someone else's actively-developed, web-facing app, so **check for upstream updates on every MINOR (`0.X.0`) or MAJOR (`X.0.0`) version bump** of Proto-Familiar — and bump for security fixes promptly. Leave it on PATCH bumps (`0.0.X`). It's a **rolling release** (no version tags), so we pin to a **commit SHA** in `searxng-service.js`. Moving the pin is deliberate, never live-tracking master: bump `SEARXNG_PIN`, confirm the `vendor/searxng-patches/*.patch` still apply (regenerate if upstream drifted), and **re-run the spawn smoke-test** (the entrypoint/bind/health assumptions can change between commits).
