...
- **Vision / media input (`media.js` + `vision.js`, spec `docs/vision-build-spec.md`, 0.9.0 = the milestone minor).** My human can show me images and I actually see them. The load-bearing rule: **`message.content` stays a plain string forever; media rides BESIDE it as an optional `attachments: [{id, kind, mime}]` sibling field.** Every string-assuming consumer (timestamp stamp/strip, char caps, memorization filters) keeps working untouched — none of them ever meets an array. Bytes live once, content-addressed, in `media/` (git-ignored). The reference becomes provider content-parts at exactly ONE seam: `materializeAttachments` in `vision.js`, called from `/api/chat` after enrichment / before the provider fetch (and, Pass 3, from `discord-gateway.js`) — that seam is the only code that knows what a model can look at, which is what makes video/audio later a materializer change, not a message-format migration. **Pass 1 (shipped, this milestone) is the spine:** the store + endpoints, the web composer (attach/paste/drag-drop, client-side downscale to 1568px), the materializer with the live budget (`visionMaxLiveImages`=4, newest-first) + capability tri-state (`visionCapable` per connection; uncached 'auto' is optimistic — the real turn is the probe, no synthetic image — with a mid-turn stand-in fallback on a modality reject), and undescribed stand-ins. **Pass 2 (not yet):** `describeAsset` (look-once-keep-forever, via `connectionForFeature(s,'vision')`), `contentWithStandins` into memorization + loop prompts, `view_image` + its gates, injection-guard on descriptions — AND the ward's chosen **image→threat scoring** (§15.1: the ward wants a shared image able to move the tier; this is a safety-critical `crisis-signals.js` change whose exact mechanism/weights get confirmed with the ward before shipping, per the safety-sign-off rule — until then `scoreMessage` stays typed-text-only). **Pass 3 (not yet):** Discord arrival-time ingest (villager images stored with provenance + room audienceTag; strangers' bytes never stored). Off-switch `PROTO_FAMILIAR_VISION_DISABLED=1` + `visionEnabled` (default ON, inert until an image is sent). Graceful degradation is absolute: no image path may 500 the chat turn — a failed store/describe/modality-reject each degrades to a visible textual absence.
