...
- **Autonomous loops**: five background workers run alongside the HTTP server. Each has a settings/env hard off-switch:
    ...
    - **Discord gateway** (`discord-gateway.js`, Village V4) — bidirectional Discord presence via bot token. A 30s supervisor follows Settings (`discordEnabled` + `discordBotToken`), so enable/disable/token changes apply without restart. Ward DMs = ward-private full context; villager DMs and @-mention guild replies run through the V3 audience gate (`audience.js`) — read `docs/village-support-design.md` before touching the router or the gate. No tools on Discord turns. Hard-disable with `PROTO_FAMILIAR_DISCORD_DISABLED=1`.
