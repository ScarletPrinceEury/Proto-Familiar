...
- **Autonomous loops**: nine background workers run alongside the HTTP server. Each has a settings/env hard off-switch:
    ...
    - **Google Calendar sync** (`gcal-sync-loop.js` + `gcal-source.js`, 0.8) — **opt-in (default OFF)**. One-way Google → Unruh, mechanical and change-gated. Wakes on a 60s base tick and syncs when the ward-configurable interval (hourly default, clamped [5min,24h]) has elapsed: a source adapter fetches the calendar (the out-of-the-box iCal-URL link tier; a **native Google account** signed in from the UI via OAuth — `gcal-google.js`, no terminal, the path to write-back; or an authenticated gogcli/gcalcli CLI — all interchangeable behind one seam, §1.5) → Unruh `gcal_ingest` parses (`ical.py`) + reconciles + **change-classifies** keyed by `gcal_uid` → only the `new` ids flag `needs_projection` and reach the Familiar (the §4 projection cue in `gcal-projection.js`, which rides chat turns, caps per turn, ages out in a few turns, and auto-clears the moment a consequence edge is added). **Deletion reconcile fires ONLY on a confirmed full snapshot with ≥1 event, and only for still-future nodes** — an empty/failed fetch or an aged-out past occurrence never cancels anything; CLI windowed reads pass `reconcile_deletes:false`. Outbound is deterministic and code-built (`icalwrite.py` → `.ics` + Google URL, local→UTC at that one boundary): `schedule_export` (any node, gated by nothing) and — opt-in, the **only** path that mutates the real calendar — `schedule_push_to_google` (ADD only, confirmed, gated behind `gcalWriteEnabled` + a CLI source). The Familiar never types a calendar artifact (§3). Toggle in Settings → "Google Calendar sync"; hard-disable with `PROTO_FAMILIAR_GCAL_DISABLED=1`. See `docs/google-integration-build-spec.md`.
